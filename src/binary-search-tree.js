...
class BinarySearchTree {
  ...
  add(data) { // добавление элементов
      this.head = addWithin(this.head, data); // положи в короень то что вернет функция (поддерево - значение)
    ...
    has(data) { // поиск или наличие
      return searchWithin(this.head, data); // поиск с главного дерева / узла + что мы ищем
  ...
  remove(data) {
      this.head = removeNode(this.head, data); // так же используем рекурсию
  ...
  min() {
      if (!this.head) { // наличие узла
      return;
      }

      let node = this.head; // переменная для поиска элемента // начинаем с корня
